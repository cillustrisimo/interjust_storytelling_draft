...
/* ===============================================
   ...
   =============================================== */
function initTimeline_Section1() {
    console.log("Initializing Section 1 Timeline...");

    const scrollContainer = document.querySelector('#section-1.scroll-space');
    const track = document.querySelector('.timeline-track');
    const lineFill = document.querySelector('.main-line-fill');

    if (!scrollContainer || !track || !lineFill) {
    ...
    fetch('timeline_data.json')
       ...
       .then(timelineData => {
            // 2. BUILD THE TIMELINE POINTS
            buildTimelinePoints(timelineData, track);
            
            // 3. RE-SELECT ELEMENTS AFTER INJECTION (they were replaced by innerHTML)
            const newLineFill = document.querySelector('.main-line-fill');
            points = document.querySelectorAll('.timeline-point');
            
            // 4. START SCROLL LOGIC
            setupScrollAnimation(scrollContainer, track, newLineFill, points);
            
            // 5. SETUP TOGGLE BUTTON EVENT DELEGATION
            setupToggleButtons();
            
        })
        .catch(error => {
            console.error("Error loading timeline data:", error);
            track.innerHTML += `<p style="color:white; padding:20px;">Failed to load timeline data. Make sure you're running on a local server.</p>`;
        });
}

// Build timeline points from data
function buildTimelinePoints(data, track) {
    let pointsHTML = '';
    
    data.forEach((item, index) => {
        pointsHTML += createPointHTML(item, index, data.length);
    });
    
    // Keep the line elements and add points
    const linesHTML = `
        <div class="main-line-bg"></div>
        <div class="main-line-fill"></div>
    `;
    
    track.innerHTML = linesHTML + pointsHTML;
}

// Create HTML for a single timeline point
function createPointHTML(item, index, total) {
    // Calculate position (distribute evenly from 5% to 95%)
    const step = 90 / (total - 1);
    const leftPos = 5 + (index * step);
    const positionClass = index % 2 === 0 ? 'point-top' : 'point-bottom';
    ...
    if (item.isVideo && item.videoSrc) {
        ...
        mediaHTML = `<img src="${item.imgSrc}" alt="${item.title}" class="card-image">`;
    }
    
    // Year marker (large background text)
    const yearHTML = item.year 
        ? `<div class="year-marker" style="${positionClass === 'point-top' ? 'top: -6rem;' : 'top: 60%;'}">${item.year}</div>` 
        : '';
    
    return `
        <div class="timeline-point ${positionClass}" style="left: ${leftPos}%;">
            <div class="dot"></div>
            ${yearHTML}
            ...
            <div class="content-card">
                ...
                <!-- Toggle button (only show if there's more text) -->
                ${item.text ? `
...
function setupScrollAnimation(scrollContainer, track, lineFill, points) {
    function updateTimeline() {
        ...
        // Update line fill
        const lineWidth = Math.min(100, progress * 100 + 2);
        lineFill.style.width = `${lineWidth}%`;
        
        // Show/hide points based on line progress
        if (points && points.length > 0) {
            points.forEach(point => {
                const pointLeftStyle = point.style.left || "0%";
                const pointLeftVal = parseFloat(pointLeftStyle);
                
                if (lineWidth > pointLeftVal) {
...
// Setup toggle button event delegation
function setupToggleButtons() {
    document.addEventListener('click', function(e) {
        if (e.target && e.target.classList.contains('toggle-btn')) {
            const moreText = e.target.nextElementSibling;
            if (moreText && moreText.classList.contains('more-text')) {
                moreText.classList.toggle('expanded');
                e.target.textContent = moreText.classList.contains('expanded') ? 'READ LESS' : 'READ MORE';
            }
        }
    });
}

// Skip timeline function - jumps to Section 2
function skipTimeline() {
    const section2 = document.getElementById('section-2');
    if (section2) {
        section2.scrollIntoView({ behavior: 'smooth', block: 'start' });
    } else {
        // Fallback: scroll past timeline if section-2 doesn't exist
        console.warn('Section 2 not found, scrolling past timeline');
        window.scrollBy({ 
            top: window.innerHeight * 12, // Scroll past the timeline
            behavior: 'smooth' 
        });
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // SECTION 1: NEW HORIZONTAL TIMELINE
